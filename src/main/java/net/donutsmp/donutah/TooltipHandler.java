...
public class TooltipHandler {
        ...
        ItemTooltipCallback.EVENT.register((stack, tooltipContext, tooltipType, lines) -> {
                    ...
                    for (String muted : DonutAH.mutedGuis) {
                        ...
                        if (screenTitle.contains(normalizedMuted)) {
                            ...
                            if (mc.currentScreen instanceof net.minecraft.client.gui.screen.ingame.HandledScreen<?> hs) {
                                ...
                                net.minecraft.screen.slot.Slot focused = acc.donutah_getFocusedSlot();
                                net.minecraft.screen.ScreenHandler handler = hs.getScreenHandler();
                                int totalSlots = handler.slots.size();
                                if (focused != null) {
                                    // Use absolute slot position (indexOf), not within-inventory index (getIndex())
            ...
            if (DonutAH.mutedItems.contains(lastHoveredItemName.toLowerCase())) {
                ...
                if (staticPrice != null) {
                    ...
                    if (DonutAHConfig.tooltipSpacer) lines.add(Text.literal(""));
                    lines.add(Text.literal(pfx + " §7Shop: " + col + "$" + formatPrice(staticPrice)));
                    int count = stack.getCount();
                    if (DonutAHConfig.showStackValue && count > 1) {
                        lines.add(Text.literal(pfx + " §7Stack §8(§f" + count + "x§8)§7: " + col + "$" + formatPrice(staticPrice * count)));
                    }
                }
                if (DonutAHConfig.devShowLabels) {
                    String label = DonutAH.itemLabels.get(lastHoveredItemName.toLowerCase());
                    if (label != null) {
                        if (staticPrice == null && DonutAHConfig.tooltipSpacer) lines.add(Text.literal(""));
                        lines.add(Text.literal(DonutAHConfig.getFormattedPrefix() + " §e★ " + label));
                    }
                }
                return;
            }
            ...
            if (BuildConstants.DEV && DonutAHConfig.devShowRawName) {
                lines.add(Text.literal("§8[dev] §7raw: §f" + itemName));
            }
            ...
            if (cached != null && !cached.isExpired()) {
                ...
                if (DonutAHConfig.tooltipSpacer) lines.add(Text.literal(""));
                lines.add(Text.literal(DonutAHConfig.getFormattedPrefix() + " §7Avg: §eFetching..."));
            } else {
                if (DonutAHConfig.tooltipSpacer) lines.add(Text.literal(""));
                lines.add(Text.literal(DonutAHConfig.getFormattedPrefix() + " §7Avg: §eFetching..."));
            }
    ...
    private static void appendShulkerValue(ItemStack shulker, List<Text> lines) {
        ContainerComponent container = shulker.get(DataComponentTypes.CONTAINER);
        if (container == null) {
            lines.add(Text.literal(DonutAHConfig.getFormattedPrefix() + " §7Contents: §7Empty"));
            return;
        }
        ...
        for (ItemStack c : container.iterateNonEmpty()) {
            hasItems = true;
        ...
        if (DonutAHConfig.tooltipSpacer) lines.add(Text.literal(""));
        if (!hasItems) {
            lines.add(Text.literal(pfx + " §7Contents: §7Empty"));
        } else if (loading) {
            lines.add(Text.literal(pfx + " §7Contents: §eFetching..."));
        } else {
            lines.add(Text.literal(pfx + " §7Contents: " + col + "$" + formatPrice(total) + " §8total"));
        }
    }
    ...
    private static void appendPriceLine(List<Text> lines, CachedPrice cached, int stackCount) {
        String pfx = DonutAHConfig.getFormattedPrefix();
        String col  = DonutAHConfig.getColorCode();

        if (cached.noData) {
            lines.add(Text.literal(pfx + " §7Avg: §cNo data"));
            return;
        }

        // Avg per unit
        lines.add(Text.literal(pfx + " §7Avg: " + col + "$" + formatPrice(cached.avgPrice) + " §8per unit"));
        ...
        if (DonutAHConfig.showStackValue && stackCount > 1) {
            ...
            lines.add(Text.literal(pfx + " §7Stack §8(§f" + stackCount + "x§8)§7: " + col + "$" + formatPrice(stackVal)));
        }

        // Min/max range
        if (DonutAHConfig.showMinMax && (cached.minPrice > 0 || cached.maxPrice > 0)) {
            lines.add(Text.literal(pfx + " §7Range: §e$" + formatPrice(cached.minPrice) + " §8→ §c$" + formatPrice(cached.maxPrice)));
        }
        ...
        if (DonutAHConfig.showTrend && cached.hasTrend) {
            ...
            lines.add(Text.literal(pfx + " §7Trend: " + arrow + " §7" + pct + " §8(24h)"));
        }

        // Daily volume
        if (DonutAHConfig.showVolume && cached.dailyVolume > 0) {
            lines.add(Text.literal(pfx + " §7Volume: §f" + cached.dailyVolume + " §8scans today"));
        }
    }
